...
import Services.BudgetService (asegurarPresupuestosPorDefecto, excedePresupuestoConNuevoMonto)
...
subMenuAgregarRegistroFinanciero = do
    ...
    mNuevo <- solicitarDatosRegistro existentes (siguienteIdRegistro existentes)
    case mNuevo of
        Nothing -> err "Operacion cancelada. No se guardo el registro."
        Just nuevo -> do
            guardarRegistros (agregarRegistro existentes nuevo)
